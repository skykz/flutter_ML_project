...
class _FeedBackScreenState extends State<FeedBackScreen> {
  ...
  final TextEditingController _textEditingControllerText =
      TextEditingController();
  ...
  Future<String> createRecord(String name, String email, String message) async {
    DocumentReference ref =
        await databaseReference.collection("feedbacks").add({
      'name': name,
      'email почта': email,
      'message': message,
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Padding(
                      padding: EdgeInsets.all(5.0), child: Text("Ваше имя: ")),
                  Container(
                      width: 200,
  ...
  Future<dynamic> sendMessage(BuildContext contextIn) async {
    if ((_textEditingControllerName.text.trim() != '') &&
        (_textEditingControllerText.text.trim() != '') &&
      ...
      if (validateEmail(email) == "true") {
        ...
        return createRecord(name, email, message).then((onValue) {
          if (onValue != null) {
            if (this.mounted)
              setState(() {
                isLoading = false;
              });
            showCustomSnackBar(contextIn, "Успешно отправлено!", Colors.green,
                Icons.check_circle_outline);
          }
        }).catchError((onError) {
          showCustomSnackBar(contextIn, "Ошибка при отправке!", Colors.red,
              Icons.info_outline);
        });
      }
      showCustomSnackBar(
          contextIn, "Введите E-mail почту!", Colors.red, Icons.info_outline);
    } else {
      showCustomSnackBar(
          contextIn, "Заполните все поля!", Colors.red, Icons.info_outline);
    }
  }
}
